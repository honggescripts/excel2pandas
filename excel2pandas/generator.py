...
# 魔法值 → 可读业务名 的示例映射。
#
# ⚠️ 这里只放「格式示范」，不要放真实业务参数。
#    真实项目的参数命名应当放在输出目录的 config_names.json 里（见 _load_config_names），
#    这样业务数据不会进入代码仓库。未命中的值由 _auto_key() 按启发式命名。
#
# 键格式由 _skey(值) 生成：小数点写成下划线，负号写成 "neg"。
#     1.13 → "1_13"      0.05 → "0_05"      -0.5 → "neg0_5"      3600 → "3600"
# ⚠️ 写成 "1.13" 是查不到的 —— 查找侧走 _skey()，键必须同格式。
...
def _render_code(model, order, results, ctx, pk_name, pk_letter, input_cols,
    ...
    A("")
    A("# 模板里的手填输入列（批量算新车时，由 inputs 覆盖 / 或用这里的默认值）")
    A("_DEFAULTS = {")
...
def _render_rules(model, results, ctx, names) -> str:
    ...
    if shared:
        ...
        A(f"  —— 请对照上面的「常量清单」逐条核对：**若这些列的业务含义不同**，说明同一个数值"
          f"被合并成了一个常量（例如 1500 同时是「渠道分成」和「运维成本」），需要拆开分别命名。")
    ...
    A("2. 想保留改动：在输出目录放一个 `config_names.json`（`{\"1_13\": \"含税系数\"}` 这种，键用下划线代替小数点），重新生成即可复用命名。")

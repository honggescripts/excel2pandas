...
⚠️ 本模板的所有数据（商品、类目、平台、金额、系数）均为编造，不含任何真实业务信息。
...
业务口径（全部虚构，仅为了让公式读起来自洽）
--------------------------------------------
单件成本 = 包装成本 + 头程运费 + 仓储费
         = 成本价×包装费率 + 类目头程运费 + 成本价×仓储费率
毛利     = 参考售价 − 平台佣金 − 单件成本 − 推广费
毛利率   = 毛利 / 参考售价
毛利投产比 = 毛利 / 推广费        （每 1 元推广费换回多少元毛利）
...
⚠️ 缓存值的算法必须与 excel2pandas/_runtime.py 的 `_round` / `_ceiling`
   **逐位一致**（同样的浮点运算顺序、同样的远离零舍入），否则对账会掉到 99%。
   所以下面不用 decimal，而是照抄运行时的 numpy 语义。
"""
...
# ---------------------------------------------------------------- Excel 语义
#
# 下面两个函数是 _runtime.py 里 _round / _ceiling 的标量克隆。
# 改动前请先看 _runtime.py —— 这里追求的不是「算得对」，是「算得一模一样」。


def xl_round(x, n=0):
    """对应 _runtime._round：sign(V) * floor(|V| * 10**n + 0.5) / 10**n"""
    f = 10.0 ** n
    return math.copysign(math.floor(abs(x) * f + 0.5) / f, x)


def xl_ceiling(x, sig):
    """对应 _runtime._ceiling：ceil(V / N) * N"""
...
V_H = xl_round(COST * 0.12, 2)                      # =ROUND(E2*0.12,2)          包装成本
V_I = RATE                                          # =VLOOKUP(D2,平台费率!$A:$B,2,0)
V_K = xl_round(V_H + V_G + COST * 0.05, 2)          # =ROUND(H2+G2+E2*0.05,2)    单件成本
V_M = xl_round(LIST * V_I, 2)                       # =ROUND(L2*I2,2)            平台佣金
V_N = xl_round(LIST - V_M - V_K - ADS, 2)           # =ROUND(L2-M2-K2-J2,2)      毛利
V_O = xl_round(V_N / LIST, 4)                       # =ROUND(N2/L2,4)            毛利率
V_P = "A" if V_N >= 30 else "B" if V_N >= 20 else "C" if V_N >= 10 else "D"
V_Q = xl_ceiling(LIST * 1.2, 5)                     # =CEILING(L2*1.2,5)         建议售价
V_R = xl_round(V_N / ADS, 2)                        # =ROUND(N2/J2,2)            毛利投产比
...
headers = [
    ...
    "头程运费", "包装成本", "佣金率", "推广费", "单件成本", "参考售价",
    "平台佣金", "毛利", "毛利率", "定价档位", "建议售价", "毛利投产比",
    "是否达标", "备注",
]
...
ws.write_formula(1, 7, "=ROUND(E2*0.12,2)", num2, V_H)
ws.write_formula(1, 8, "=VLOOKUP(D2,平台费率!$A:$B,2,0)", pct, V_I)
ws.write_formula(1, 10, "=ROUND(H2+G2+E2*0.05,2)", num2, V_K)
...
ws.write_formula(1, 17, "=ROUND(N2/J2,2)", num2, V_R)
...
print("  外部表「运费价目」/「平台费率」")
print("\n样板行缓存值：")
for col, val in zip("GHIKMNO P Q R S".replace(" ", ""),
                    [V_G, V_H, V_I, V_K, V_M, V_N, V_O, V_P, V_Q, V_R, V_S]):
    print(f"  {col}2 = {val!r}")

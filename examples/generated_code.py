...
# 模板里的手填输入列（批量算新车时，由 inputs 覆盖 / 或用这里的默认值）
_DEFAULTS = {
    "商品名称": '示例商品',
    "类目": '家居',
    "平台": '平台A',
...
def _n(x):
    """Excel 的隐式数值转换：'7.5%' -> 0.075、'3,900' -> 3900。
...
def calc(primary_values, file_paths=None, inputs=None):
    ...
    # K2: =ROUND((H2+G2+F2*0.05)/F2,2)
    df['单件成本'] = _round(_div(((((df['含税成本']) + (df['头程运费']))) + (((df['采购数量']) * (CONFIG['_rate_0_05'])))), df['采购数量']), 2)
    ...
    # Q2: =CEILING(L2*1.2,5)
    df['建议售价'] = _ceiling(((df['参考售价']) * (CONFIG['_rate_1_2'])), 5)

    # R2: =ROUND(PMT(0.06/12,12,-L2),2)
    df['月供测算'] = _round(_pmt(((CONFIG['年化费率']) / (12)), 12, (-df['参考售价'])), 2)

    # S2: =IF(O2>=0.3,"是","否")
    df['是否达标'] = _if(((df['毛利率']) >= (CONFIG['_rate_0_3'])), '是', '否')
